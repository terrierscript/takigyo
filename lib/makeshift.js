...
function createMarkup(selectors){
  var markups = selectors.map(function(selector){
    return migawari(selector, function($elm, currentSelector){
      $elm.attr("data-selector", selector)
    })
  })
...
function createHtml(markup, style, script){
  ...
  tags.push("<html>", "<head>")
  if(style){
    tags.push("<style>" , style , "</style>")
  }
  tags.push("<body>", markup)
  if(script){
    tags.push("<script>" , script , "</script>")
  }
  tags.push("</body>")
  tags.push("</head>", "</html>")
  return tags.join("\n")
